package cn.edu.swust.serialport.ui;
...
import cn.edu.swust.serialport.manager.SerialPortManager;
import cn.edu.swust.serialport.utils.ByteUtils;
import cn.edu.swust.serialport.utils.ShowUtils;
...
public class MainFrame extends JFrame {
	...
	private JScrollPane mScrollDataView = new JScrollPane(mDataView);//滚动面板，文档P135
	...
	private JComboBox mCommChoice = new JComboBox();//下拉列表框（选择串口）
	private JComboBox mBaudrateChoice = new JComboBox();//下拉列表框（波特率选择）
	private ButtonGroup mDataChoice = new ButtonGroup();//按钮组，文档P74；当有多个单选按钮时，一般只允许一个单选按钮选中， 因此需要对同一类型的单选按钮进行分组
	...
	private void initData() {
		...
		// 检查是否有可用串口，有则加入选项中
		if (mCommList == null || mCommList.size() < 1) {//为什么要这样判断，只要一个不行吗？****************************************************************************************************
	...
	private void actionListener() {
		/*
		mCommChoice.addItemListener(new ItemListener() {
			
			@Override
			public void itemStateChanged(ItemEvent e) {
				// TODO Auto-generated method stub
				mCommList=SerialPortManager.findPorts();//寻找
				if(mCommList==null || mCommList.size()<1){
					ShowUtils.warningMessage("没有搜索到串口");
				}else {
	...
	private void openSerialPort(ActionEvent evt) {
		...
		// 获取波特率，默认为9600
		int baudrate = 9600;//如何默认出来的？***********************************************************************************
		String bps = (String) mBaudrateChoice.getSelectedItem();
		baudrate = Integer.parseInt(bps);

		// 检查串口名称是否获取正确
		if (commName == null || commName.equals("")) {//什么情况下为“”********************************************************************
			ShowUtils.warningMessage("没有搜索到有效串口！");
		} else {
			try {
	...
	private void sendData(ActionEvent evt) {
		// 待发送数据
		String data = mDataInput.getText().toString().trim();//toString()多余了？***************************************************************************
